...
import { cellAddressFromString, columnAddressFromString, rowAddressFromString, cellAddressFromImmutableReference, rowAddressFromImmutableReference, colAddressFromImmutableReference } from './addressRepresentationConverters';
...
export class FormulaParser extends EmbeddedActionsParser {
  constructor(lexerConfig, sheetMapping, immutableReferenceMapping) {
    ...
    this.immutableColumnRangeExpression = this.RULE('immutableColumnRangeExpression', () => {
      const range = this.CONSUME(ImmutableColRange);
      const [startImage, endImage] = range.image.split(':');
    ...
    this.immutableRowRangeExpression = this.RULE('immutableRowRangeExpression', () => {
      const range = this.CONSUME(ImmutableRowRange);
      const [startImage, endImage] = range.image.split(':');
    ...
    this.immutableCellReference = this.RULE('immutableCellReference', () => {
      const cell = this.CONSUME(ImmutableCellReference);
    ...
    this.endImmutableRangeReference = this.RULE('endImmutableRangeReference', start => {
      var _a;
      const end = this.CONSUME(ImmutableCellReference);
      const startAddress = this.ACTION(() => {
